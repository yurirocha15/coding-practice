...
class Solution:
    def secondMinimum(
        ...
        for s, e in edges:
            graph[s].append(e)
            graph[e].append(s)
        min_val, sec_min = [float("inf")] * n, [float("inf")] * n
        min_val[0] = 0
        ...
        while queue:
            ...
            for child in graph[cur_node]:
                q, r = divmod(cur_time, change)
                next_time = cur_time + time + (change - r if q % 2 == 1 else 0)
                if min_val[child - 1] > next_time:
                    sec_min[child - 1], min_val[child - 1] = (
                        min_val[child - 1],
                        next_time,
                    )
                    queue.append((next_time, child))
                elif sec_min[child - 1] > next_time and next_time != min_val[child - 1]:
                    sec_min[child - 1] = next_time
                    queue.append((next_time, child))
        return sec_min[n - 1]
