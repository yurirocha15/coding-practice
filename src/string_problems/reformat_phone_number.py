...
Then, group the digits from left to right into
blocks of length 3 until there are 4 or fewer digits.
...
class Solution:
    def reformat_number(self, number: str) -> str:
        ...
        n, ret, i = len(number), "", 0
        # split numbers 3 by 3 until we have 4 or less numbers left
        while i < n - 4:
            ret += number[i : i + 3] + "-"
            i += 3
        # if we have 4 numbers left split into two
        if i == n - 4:
            ret += number[i : i + 2] + "-" + number[i + 2 :]
        # otherwise add the remaining numbers
        else:
            ret += number[i:]
        return ret
...
if __name__ == "__main__":
    ...
    assert solution.reformat_number("--17-5 229 35-39475 ") == "175-229-353-94-75"
